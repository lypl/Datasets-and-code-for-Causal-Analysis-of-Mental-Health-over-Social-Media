...
''' 根据各个template_mapping 构造对应的config,还需要在得到ckpt后获得ckpt-path：xxx,并复制多条在多个ckpt上推断,换成key_stc 和 not_key_stc两个分类 '''
for i in range(5):
    now_config = []
    for j in range(5):
        ckpt = int(500*(j+1))
        now_config.append({
            ...
            "pretrained_model": "/media/data/3/lyp/stc_score_ckpt/"+str(i)+"/checkpoint-"+str(ckpt),
            "batch_size": 2,
            "multiclass": True,
            "use_cuda": True,
            "half": True,
            ...
            "template_mapping": all_tpl_mappings[i],
        })
    config_path = os.path.join(os.getcwd(), 'configs', "config_"+str(i)+".json")
