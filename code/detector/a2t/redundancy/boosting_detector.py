...
for file_name in file_list:
    ...
    with open(path, 'r') as f:
        ...
        templates_ori_list.append(tpl)
''' template 个数实验 '''
template_num = int(sys.argv[1])
print("template_num")
print(template_num)
rdm_idxs = random.sample(range(len(templates_ori_list)), template_num)
for idx in rdm_idxs:
    templates_list.append(templates_ori_list[idx])
...
out_dir = os.path.join("/media/data/3/lyp/stc_score_ckpt/boosting", "template_num", str(template_num))
os.makedirs(out_dir, exist_ok=True)
np.save(os.path.join(out_dir, "data_weight.npy"), data_weights)
np.save(os.path.join(out_dir, "learner_weight.npy"), np.array(learner_weights))
weak_learner_path = os.path.join(out_dir, "weak_learners.json")
