import collections, logging, math, os, time, torch, numpy
...
class Trainer:
    def __init__(self, model, args, data_collator, train_dataset=None, eval_dataset=None, tokenizer=None,
        ...
        if self.args.should_save:
            ...
            if self.args.proto_output_dir != "eval":
                os.makedirs(self.args.proto_output_dir, exist_ok=True)
        self.use_amp = True
    ...
    def train(self):
        ...
        for epoch in range(num_train_epochs):
            ...
            # 输出proto到args.output_dir/proto.npy
            if self.args.proto_output_dir != "eval" and hasattr(model, "prototype"):
                proto_weight = model.prototype.proto.detach().cpu().numpy()
                output_path = os.path.join(self.args.proto_output_dir, 'proto_epoch_{}.npy'.format(str(epoch)))
                numpy.save(output_path, proto_weight)
