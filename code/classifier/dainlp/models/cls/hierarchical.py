...
from dainlp.modules.utils import get_sinusoidal_embeddings, ContrastiveLoss
from dainlp.modules.seq2vec.lwan import MullenbachModel, Prototype


'''[2022-Mar-13] classification with classifier head'''
...
''' classification with prototype (not use threshold to classify "no reason"), rather classifier head '''
class Model(RobertaPreTrainedModel):
    def __init__(self, config):
        super(Model, self).__init__(config)
        self.num_labels = config.num_labels
        ...
        self.projector = torch.nn.Linear(config.hidden_size, config.proto_dim)
        self.prototype = Prototype(config.proto_dim, config.num_labels)
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None, labels=None):
        ...
        # logits = self.prototype(hidden_states) # 加projector的版本,和下面的2选1，⭐这时上面的网络结构也要注释
        logits = self.prototype(self.projector(hidden_states))
        ...
        if self.config.task_name == "singlelabel":
            ...
            loss_fct = torch.nn.BCEWithLogitsLoss()
            loss = loss_fct(logits, labels)
            pcl_loss = ContrastiveLoss(hidden_states, logits, labels, self.num_labels, tau=self.config.tau)
            loss = loss + pcl_loss
            # no use
            # loss = (1.0 - self.config.alpha)* loss + self.config.alpha * pcl_loss
            # loss = pcl_loss
