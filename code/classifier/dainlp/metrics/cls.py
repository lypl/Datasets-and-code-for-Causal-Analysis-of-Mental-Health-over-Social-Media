...
from dainlp.metrics.utils import F_Score, auc_metrics, precision_at_k, my_accuracy_score
...
class Metric:
    ...
    def __call__(self, all_logits, all_golds):
        ...
        if self.task_name == "singlelabel":
            ...
            metrics.update(my_accuracy_score(all_logits, all_golds))
            # metrics.update({f"macro_{k}": v for k, v in f_score.get_micro_f1_by_class().items()}) 
            # metrics.update({f"macro_{k}": v for k, v in f_score.get_macro_avg_scores().items()})
